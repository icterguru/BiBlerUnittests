'''
TODO: Assignment 3: This module must be automatically generated.
'''
# Python source code generated by SCC (StateChart Compiler) 0.3, written by Thomas Feng
#   Source: D:/Documents/My Documents/Alabama/Development/workplaces/WAToM3/atom3-original/atom3/User/User Models\BiBler_Statechart.des
...
class BiBler_Statechart(StateMachine):
  ...
  ParentTable=[-1,  # Exit -- parent (None)
               -1,  # Run -- parent (None)
               1,  # Run.Idle -- parent Run
               -1   # Start -- parent (None)
              ]
  HistoryStateTable=[0,
                     0,
  ...
  Hierarchy=[[0, 0, 0, 0],  # substates of state Exit
             [0, 0, 1, 0],  # substates of state Run
             [0, 0, 0, 0],  # substates of state Run.Idle
             [0, 0, 0, 0]   # substates of state Start
            ]
  ...
  def __init__(self, Interpreter=None, Parent=None, OldInstance=None):
    ...
    if OldInstance!=None:
      ...
      self.Submodels=[]
      for i in range(BiBler_Statechart.StateNum):
    ...
    for i in range(BiBler_Statechart.StateNum):
      ...
      for j in range(BiBler_Statechart.StateNum):
    ...
    self.description=BiBler_Statechart.Description

  # Methods
  def isParent(self, sp, sc):

    return sc>=0 and (sp<0 or BiBler_Statechart.Hierarchy[sp][sc])
  ...
  def isInState(self, s, check_substate=1, use_backup=1):
    ...
    if isinstance(s, int):
      ...
      i=0
      while i<BiBler_Statechart.StateNum:
        if s==BiBler_Statechart.StateNames[i]:
          return self.isInState(i, check_substate, use_backup)
        i=i+1
      i=0
      while i<BiBler_Statechart.StateNum:
        stname=BiBler_Statechart.StateNames[i]
  ...
  def initModel(self, run_initializer=1, run_enter_actions=1):
    ...
    self.clearEnteredStates()
    if self.Parent==None and BiBler_Statechart.Description!=None:
      sys.__stdout__.write(BiBler_Statechart.Description+"\n")
    self.addInState(3) # init state "Start"
  ...
  def handleEvent(self, se, params=[], cond=None, scheduler=None):
    ...
    if e==0: # event "__INTERNAL_0_TIME_0"
      if table[2] and self.isInState(2) and self.testCondition(0):
        if (scheduler==self or scheduler==None) and table[2]:
          ...
          self.applyMask(BiBler_Statechart.OrthogonalTable[2], table)
          handled=1
    elif e==1: # event "exitClicked"
      if table[1] and self.isInState(1) and self.testCondition(1):
        if (scheduler==self or scheduler==None) and table[1]:
          self.runExitActionsForStates(-1)
          self.clearEnteredStates()
          self.changeState(1, 0)
          self.runEnterActionsForStates(self.StatesEntered, 1)
          self.applyMask(BiBler_Statechart.OrthogonalTable[1], table)
          handled=1
    elif e==2: # event "start"
      if table[3] and self.isInState(3) and self.testCondition(2):
        if (scheduler==self or scheduler==None) and table[3]:
          ...
          self.runEnterActionsForStates(self.StatesEntered, 1)
          self.applyMask(BiBler_Statechart.OrthogonalTable[3], table)
          handled=1
    if cond and self.Parent==None:  # Top-level model
      cond.release()
    if not handled and e>=0 and (scheduler==self or scheduler==None) and BiBler_Statechart.RescheduleTable[e]>=0:
      self.addSchedule(BiBler_Statechart.RescheduleTable[e], BiBler_Statechart.IntervalTable[e], BiBler_Statechart.EventNames[e], scheduler)
    return handled
  ...
  def changeState(self, s1, s2, check_history=0, top_level=0):
    ...
    # t1=common(s1, s2)
    t1=BiBler_Statechart.CommonStateTable[s1][s2]
    ...
    if t1>=0:
      ...
      self.state=None
    # t2=history(s2)
    t2=BiBler_Statechart.HistoryStateTable[s2]
    if t2==0: # no history
      ...
      if dest==0:
        if history_type!=2 or self.check_history(-1):
          if history_type!=2 or self.check_history(0):
            ...
            self.addInState(0)  # move into leaf state "Exit"
      elif dest==1:
        if history_type!=2 or self.check_history(-1):
          if history_type!=2 or self.check_history(1):
            ...
            if history_type!=2 or self.check_history(2):
              ...
              self.addInState(2)  # move into leaf state "Run.Idle"
      elif dest==2:
        if history_type!=2 or self.check_history(-1):
          if history_type!=2 or self.check_history(1):
            self.recordEnteredState(1)
        ...
        if history_type!=2 or self.check_history(-1):
          if history_type!=2 or self.check_history(3):
            self.recordEnteredState(3)
            self.addInState(3)  # move into leaf state "Start"
    elif common==0:
      if dest==0:
        if history_type!=2 or self.check_history(0):
          self.addInState(0)  # move into leaf state "Exit"
  ...
  def eventStr2Int(self, event):
    ...
    for i in range(3):
      if event==BiBler_Statechart.EventNames[i]:
        return i
    return -1
  ...
  def stateInt2Str(self, state):
    ...
    if state==-1:
      ...
      return BiBler_Statechart.StateNames[state]
  ...
  def getParentState(self, state):
    ...
    return BiBler_Statechart.ParentTable[state]
  ...
  def getSubstates(self, state):
    ...
    if state==-1: # substates of ""
      ...
      st.Next=substates
      substates=st
    elif state==0: # substates of "Exit"
      pass
    elif state==1: # substates of "Run"
      # add substate "Run.Idle"
      st=IntList()
      st.int=2
      st.Next=substates
      substates=st
    elif state==2: # substates of "Run.Idle"
      pass
    elif state==3: # substates of "Start"
      pass
    return substates

  def isHistoryState(self, state):

    return BiBler_Statechart.HistoryStateTable[state]>0

  def isLeafState(self, state):

    if isinstance(state, int):
      return BiBler_Statechart.LeafStateTable[state]!=None

    elif isinstance(state, str):
      for i in range(BiBler_Statechart.StateNum):
        if BiBler_Statechart.LeafStateTable[i]==None:
          continue
        if state==BiBler_Statechart.LeafStateTable[i] and self.Submodels[i]==None:
          return 1
        elif startsWith(state, BiBler_Statechart.LeafStateTable[i]+".") and self.Submodels[i]!=None:
          SubmodelState=state[BiBler_Statechart.LeafStateTable[i].length()+1:]
  ...
  def hasOrthogonalStateInBetween(self, parent, leaf):
    ...
    return BiBler_Statechart.OrthogonalInBetween[parent+1][leaf]
  ...
  def getHierarchy(self, start_level, state_prefix):
    ...
    lasth=lasth.Next
    # Generate state "Run" in the hierarchy table
    lasth.Next=Hierarchy()
    lasth.Next.StateName="Run"
    if state_prefix==None:
      lasth.Next.PathName="Run"
    else:
      lasth.Next.PathName=state_prefix+".Run"
    lasth.Next.StateNum=1
    lasth.Next.Level=start_level+0
    lasth=lasth.Next
    # Generate state "Run.Idle" in the hierarchy table
    lasth.Next=Hierarchy()
    lasth.Next.StateName="Idle"
    if state_prefix==None:
      lasth.Next.PathName="Run.Idle"
    else:
      lasth.Next.PathName=state_prefix+".Run.Idle"
    lasth.Next.StateNum=2
    lasth.Next.Level=start_level+1
    lasth=lasth.Next
    # Generate state "Start" in the hierarchy table
    lasth.Next=Hierarchy()
    lasth.Next.StateName="Start"
    if state_prefix==None:
      lasth.Next.PathName="Start"
    else:
      lasth.Next.PathName=state_prefix+".Start"
  ...
  def runActionCode(self, code_num):
    ...
    if code_num==0: # model finalizer
      ...
      self.addSchedule(2, BiBler_Statechart.IntervalTable[0], BiBler_Statechart.EventNames[0], self)
    elif code_num==3: # output action(s) of a transition
      self.runCode("controller=eventhandler.get_event_params()")
    elif code_num==4: # enter actions for state "Exit"
      self.runCode("controller.exit()")
    elif code_num==5: # enter actions for state "Run.Idle"
      self.runCode("dump_message(\'Statechart is idle\')")
      # a timed transition
      self.addSchedule(2, BiBler_Statechart.IntervalTable[0], BiBler_Statechart.EventNames[0], self)
    elif code_num==6: # exit actions for state "Run.Idle"
  ...
  def runEnterActions(self, state):
    ...
    if state==0: # enter action(s) for state "Exit"
      self.runActionCode(4)
    elif state==2: # enter action(s) for state "Run.Idle"
      self.runActionCode(5)

  def runExitActions(self, state):

    if state==2: # exit action(s) for state "Run.Idle"
...
if __name__=="__main__":
  BiBler_Statechart.main(sys.argv)